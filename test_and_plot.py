...
for name, checkpoint, solver in checkpoints:
	os.system(f"python driver.py test --2d --train_abstraction --checkpoint checkpoints/{checkpoint} --solvers {solver} --timeout 20 --outputFolder {name}{timestamp} --maxShapes 30")
...
#sbatch execute_gpu_1080.sh python driver.py imitation --2d --train_abstraction --contrastive --nonmodular --checkpoint nonmodularAbstractContrastiveNoSpecInOE.pickle --trainTime 48
