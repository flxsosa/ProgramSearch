...
"""
from https://github.com/insperatum/pinn

added beam search, and modified so it can handle an env

IMPORTANT: because buttons are unhashable, changed target_vocabulary_index to name-based  
"""
...
class RobustFill(nn.Module):
    ...
    def _refreshVocabularyIndex(self):
        ...
        self.target_vocabulary_index = {self.target_vocabulary[j].name: j for j in range(len(self.target_vocabulary))}
    ...
    def _targetToTensor(self, targets):
        ...
        for i in range(len(targets)):
            s = targets[i]
            if len(s)>0: t[:len(s), i] = torch.LongTensor([self.target_vocabulary_index[x.name] for x in s])
        return t
